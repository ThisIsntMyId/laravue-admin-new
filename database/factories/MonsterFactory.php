...
$factory->define(Monster::class, function (Faker $faker) {
    ...
    $fav_food = Food::all()->random($faker->numberBetween($min=3, $max=9))->map(function ($food, $key) {
        return $food->id;
    })->implode(',');
    $nature = Nature::all()->random($faker->numberBetween($min=3, $max=9))->map(function ($food, $key) {
        return $food->id;
    })->implode(',');
    $location = Location::all()->random($faker->numberBetween($min=3, $max=9))->map(function ($food, $key) {
        return $food->id;
    })->implode(',');
    $nameIndex = $faker->numberBetween($min = 0, $max = 49);
    return [
        'name' => '{"en":"' . $names[$nameIndex] . '", "ja":"'.$japanese_names[$nameIndex] . '"}',
        // 'japanese_name' => $japanese_names[$nameIndex],
        'description' => '{"en":"' . $faker->paragraph . '", "ja":"'. $japanese_description[$faker->numberBetween($min = 0, $max = 5)] . '"}',
